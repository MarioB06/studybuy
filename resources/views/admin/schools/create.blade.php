<x-app-layout>
    ...
    <div class="admin-container">
        <h1 style="font-size:32px;font-weight:600;margin-bottom:30px">Neue Schule</h1>
        <div class="form-card">
            <form method="POST" action="{{ route('admin.schools.store') }}">
                ...
                <div class="form-group">
                    ...
                    <input type="text" name="country" class="form-input" value="{{ old('country', 'CH') }}" maxlength="2">
                    @error('country')<div style="color:#e74c3c;font-size:14px;margin-top:5px">{{ $message }}</div>@enderror
                </div>
                <button type="submit" class="btn btn-primary">Schule erstellen</button>
            </form>
        </div>
    </div>
</x-app-layout>
